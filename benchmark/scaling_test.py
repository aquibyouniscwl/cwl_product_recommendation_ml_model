...
def generate_payload(top_k):
    ...
    enrolled_products = random.sample(
        PRODUCT_IDS,
        enrolled_size,
    )
...
async def make_request(
    ...
    try:
        ...
        async with session.post(
            ...
            cache_status = response_json.get(
                "cache_status",
                "unknown"
            )
...
async def run_scalability_test(
    ...
    memory_task = asyncio.create_task(
        monitor_memory(memory_snapshots)
    )
    ...
    # =================================================
    avg_memory = (
        statistics.mean(memory_snapshots)
        if memory_snapshots
        else 0
    )

    peak_memory = (
        max(memory_snapshots)
        if memory_snapshots
        else 0
    )
    ...
    redis_hit_ratio = (
        (len(cache_hits) / total_processed) * 100
        if total_processed > 0
        else 0
    )

    lru_hit_ratio = (
        (len(lru_hits) / total_processed) * 100
        if total_processed > 0
        else 0
    )
    ...
    return {
        ...
        "lru_hit_ratio_percent":
            round(lru_hit_ratio, 2),

        # Combined
        "top_pattern_hit_rate_percent":
            round(overall_cache_ratio, 2),
