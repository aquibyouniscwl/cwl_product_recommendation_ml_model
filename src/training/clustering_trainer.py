...
class ClusteringTrainer:
    def train_and_save(self, vectors: np.ndarray, n_clusters: int = 3) -> KMeans:
        ...
        """
        logger.info(f"Starting KMeans clustering training with n_clusters={n_clusters}...")
        
        # Instantiate and fit KMeans
        kmeans = KMeans(n_clusters=n_clusters, random_state=42, n_init=10)
        kmeans.fit(vectors)
        logger.info(f"KMeans model trained. Cluster centers shape: {kmeans.cluster_centers_.shape}")
