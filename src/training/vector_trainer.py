...
class VectorTrainer:
        ...
        for idx, item in enumerate(data):
            ...
            for field, expected_type in required_fields.items():
                if field not in item:
                    logger.error(f"Validation failed: Product at index {idx} is missing required field '{field}'")
                    return False
                val = item[field]
            ...
            if item["difficulty"] not in ["beginner", "intermediate", "advanced"]:
                logger.error(f"Validation failed: Product ID '{item['id']}' has invalid difficulty value '{item['difficulty']}'")
                return False
                
        logger.info(f"Dataset validation successful. Total validated products: {len(data)}")
        return True
    ...
    def train_and_save(self) -> Tuple[np.ndarray, pd.DataFrame]:
        ...
        if not os.path.exists(settings.RAW_DATASET_PATH):
            raise FileNotFoundError(f"Raw dataset not found at: {settings.RAW_DATASET_PATH}")
        ...
        technologies_features = self.technologies_encoder.fit_transform(df["technologies"])
        tools_features = self.tools_encoder.fit_transform(df["tools"])
        related_domains_features = self.related_domains_encoder.fit_transform(df["relatedDomains"])
        learning_path_features = self.learning_path_encoder.fit_transform(df["learningPath"])
        ...
        weighted_related_domains = related_domains_features * FEATURE_WEIGHTS["relatedDomains"]
        weighted_learning_path = learning_path_features * FEATURE_WEIGHTS["learningPath"]
        ...
        logger.info(f"Generated vectors with combined feature shape: {combined_features.shape}")
        ...
        # Dump encoders
        joblib.dump(self.team_encoder, os.path.join(settings.MODELS_DIR, "team_encoder.pkl"))
        joblib.dump(self.security_encoder, os.path.join(settings.MODELS_DIR, "security_encoder.pkl"))
        joblib.dump(self.topics_encoder, os.path.join(settings.MODELS_DIR, "topics_encoder.pkl"))
        joblib.dump(self.tags_encoder, os.path.join(settings.MODELS_DIR, "tags_encoder.pkl"))
        joblib.dump(self.technologies_encoder, os.path.join(settings.MODELS_DIR, "technologies_encoder.pkl"))
        joblib.dump(self.tools_encoder, os.path.join(settings.MODELS_DIR, "tools_encoder.pkl"))
        joblib.dump(self.related_domains_encoder, os.path.join(settings.MODELS_DIR, "related_domains_encoder.pkl"))
        joblib.dump(self.learning_path_encoder, os.path.join(settings.MODELS_DIR, "learning_path_encoder.pkl"))
        joblib.dump(self.domain_encoder, os.path.join(settings.MODELS_DIR, "domain_encoder.pkl"))
        joblib.dump(self.scaler, os.path.join(settings.MODELS_DIR, "scaler.pkl"))

        logger.info("Saved product vectors, encoders, metadata, and cleaned outputs successfully.")
